...
// nilIfEmpty collapses an empty slice to nil, so "no values" has one
// representation across both backends. A caller cannot tell the two apart by
// reading them; reflect.DeepEqual can, which is the whole point of the
// differential suite.
...
func (j computerJSON) model() (*Computer, error) {
	...
	c := &Computer{
		...
		// An absent multi-valued attribute is nil, not an empty slice. JSON
		// decoding produces an empty one for "[]", which is the same thing to
		// anything that reads it but a different Go value — enough to make
		// this model unequal to go-adldap's for an identical object, which
		// the cross-backend differential suite reports as a disagreement.
		ServicePrincipalNames: nilIfEmpty(j.ServicePrincipalNames), AllowedToDelegateTo: nilIfEmpty(j.AllowedToDelegateTo),
		PrincipalsAllowed: nilIfEmpty(j.PrincipalsAllowed), KerberosEncryptionType: nilIfEmpty(j.KerberosEncryptionType),
